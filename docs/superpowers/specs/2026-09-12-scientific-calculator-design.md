...
- `angleUnit`：可选，缺省取配置
- `variables`：可选，**请求级临时变量**，仅本次求值生效、不落库；同名时覆盖存储中的变量。**此处的 `variables` 允许使用保留常量名**（如 `{"pi": 3}`），因为它不写入存储，不会破坏后续表达式的可重现性
...
| `CalculationServiceTest` | 历史落库、请求级变量覆盖、**请求级变量允许用保留常量名**、角度单位传递 |
...
| **D4** | v1 允许用户变量覆盖内置常量（`pi = 3`） | **改为保留名** | 覆盖会让 `sin(pi)` 的含义随写入操作静默改变，破坏表达式可重现性。常量属于语言，不属于缺省值。**连带简化**：变量名禁用集合统一为"函数名 ∪ 常量名" |
