...
**这一节是数值能力的对外承诺，也是「哪些输入不必完美处理」的边界。** 范围之内的输入保证算得对；范围之外一律**拒收**——抛 `CalcException` 走 4xx，不保证给出哪个错误码，也不承诺优雅降级。刻意不追求「对任意极端输入都给出有意义结果」：那种目标没有上界，而计算器的价值在于对**常用域**精确可靠。

#### 5.3.1 支持范围（范围内保证正确）

| 维度 | 上界 | 出处 |
|---|---|---|
| 精确十进制数的 scale | `\|scale\| ≤ 100 000` | `DecimalNumber.MAX_SCALE_MAGNITUDE` |
| 精确幂的未缩放位数 | `≤ 100 000` 位 | `Numbers.MAX_EXACT_DIGITS` |
| 阶乘自变量 | 非负整数且 `≤ 170` | `Numbers.FACTORIAL_LIMIT` |
| 浮点结果 | 有限且规整到 **15 位有效数字** | `Numbers.NORMALIZE_CONTEXT` |
...
超出上表的输入**只需抛异常**，实现者不得为此扩增边界处理代码：

- 精确幂超预算 → 降级到 `double`，由 `requireFinite` 兜底（这是 §5.2 规定的**降级**，属于范围内行为，不是边界修补）；
- 非有限结果（`NaN`/`±Inf`） → `NON_FINITE_RESULT`；
- scale 越界、阶乘超上界、非整数阶乘 → `NON_FINITE_RESULT` / `DOMAIN_ERROR`；
- 表达式超长 → `INVALID_REQUEST`；
- 其余未枚举的极端形态 → 任一 `CalcException` 子类即可，**不要求逐一穷举、不要求专门的错误语义**。

**允许的实现**：值对象在构造时自查量级不变量，越界即构造失败——一个不让自身处于非法状态的值对象，比每个运算各自兜底更易读（§5.3.3）。

#### 5.3.3 为什么把边界写成契约而不是「尽量支持」

1. **无界目标不可验收。** 「支持任意大的数」既做不到（内存是有限的），也无法写成测试；而「`|scale| ≤ 100000`」可以，且能精确断言边界两侧。
2. **降级优于伪装。** 超出精确预算时降级到 `double` 会损失精度但仍在 `double` 域内，比返回一个几百万位的中间结果更符合「计算器」的用途。
3. **拒收优于崩溃。** 契约要求范围外**抛领域异常**（`CalcException`，映射为 4xx），而不是让 `BigDecimal` 的 `ArithmeticException`/`NumberFormatException` 逃到接口层变成 500——后者是「未处理的缺陷」，前者是「明确的拒绝」。
4. **边界收口在值对象。** 量级约束放在 `DecimalNumber` 的紧凑构造器里，五个运算的结果路径一次性被覆盖；逐个运算写 `try`/`catch` 会把同一条不变量重复五六遍，且每次都要新编一套错误语义。
...
禁用集合在两个入口处被拒收，因此求值期两个集合不相交。**IDENT 的解析规则是常量优先**：
先查保留常量，未命中则查用户变量，仍未命中抛 `UNKNOWN_VARIABLE`。求值器刻意**不依赖**
