...
| `variables` | 否 | 请求级临时变量，**只对本次求值生效、不落库**。键必须是合法标识符且**不得落在禁用集合**（23 个一元函数名 ∪ 5 个二元函数名 ∪ `pi` `e`）内，否则 400 `INVALID_REQUEST`；值为 `null` 同样拒收 |
...
| 标识符 | `[A-Za-z_][A-Za-z0-9_]*`，用于函数名、常量名（`pi` / `e`）与请求级变量名 |
| 括号 | `( )`，可任意嵌套 |
| 算符 | `+ - * / % ^`（中缀）、前缀 `+` `-`、后缀 `!`（阶乘） |
